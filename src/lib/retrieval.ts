...
/** Boost score for nodes from the last N days */
...
function matchReason(keywordScore: number, semanticScore: number): string {
  const parts: string[] = [];
  if (keywordScore >= CONTEXT_MATCH_THRESHOLD) {
    parts.push("keyword overlap");
  }
  if (semanticScore >= SEMANTIC_MATCH_THRESHOLD) {
    parts.push("semantic similarity");
  }
  if (parts.length === 0) return "weak signal";
  return parts.join(" + ");
}

function scoreNodeHybrid(
  prompt: string,
  node: KnowledgeNode,
  ...
  let semanticScore = 0;
  if (queryEmbedding?.length && nodeEmbeddings?.[node.id]?.length) {
    semanticScore = cosineSimilarity(queryEmbedding, nodeEmbeddings[node.id]);
  }
  ...
  const combinedScore =
    keywordScore + semanticScore * 10 + recencyBoost(node, now);
...
/** Stage 1: keyword-ranked prefilter. Stage 2: hybrid on candidates (+ recent fallback). */
export function scorePromptHybridTopK(
  ...
  if (candidates.length < PREFILTER_TOP_K) {
    const byRecency = [...filtered].sort((a, b) => b.date - a.date);
  ...
  for (const node of candidates) {
    const match = scoreNodeHybrid(
      prompt,
      node,
      options?.queryEmbedding,
      options?.nodeEmbeddings,
      now
    );
...
export function formatMergedContextInjection(
  matches: MatchResult[] | KnowledgeNode[]
): string {
  const nodes: KnowledgeNode[] =
    matches.length > 0 && "node" in matches[0]
      ? (matches as MatchResult[]).map((m) => m.node)
      : (matches as KnowledgeNode[]);

  if (nodes.length === 0) return "";
  if (nodes.length === 1) {
    return `${formatContextInjection(nodes[0])}\n---`;
  }
  ...
  for (const node of nodes) {
    lines.push(`### ${node.topic} (${node.platform}, ${new Date(node.date).toLocaleDateString()})`);
